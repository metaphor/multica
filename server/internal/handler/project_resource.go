...
func (c claimProjectContext) applyTo(resp *AgentTaskResponse) {
	...
	resp.Repos = c.Repos
	resp.EnableAgentWorkdir = c.EnableAgentWorkdir
	resp.AgentWorkdir = c.AgentWorkdir
}
...
func (h *Handler) resolveClaimProjectContext(ctx context.Context, projectID, workspaceID pgtype.UUID) (claimProjectContext, error) {
	...
	if projectID.Valid {
		...
		case err == nil:
			...
			// Only advertise a custom per-project workdir when the project has no
			// local_directory resources; a local_directory resource takes precedence
			// because it already pins the task to a specific user directory.
			if len(project.Settings) > 0 {
				...
				if !hasLocalDirectory {
					...
					if json.Unmarshal(project.Settings, &s) == nil {
						out.EnableAgentWorkdir = s.EnableAgentWorkdir
						out.AgentWorkdir = s.AgentWorkdir
					}
				}
			}
