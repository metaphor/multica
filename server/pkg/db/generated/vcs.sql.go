...
const getIssueVCSPullRequestLink = `-- name: GetIssueVCSPullRequestLink :one
SELECT pull_request_id FROM issue_vcs_pull_request
WHERE issue_id = $1 AND pull_request_id = $2
`

type GetIssueVCSPullRequestLinkParams struct {
	IssueID       pgtype.UUID `json:"issue_id"`
	PullRequestID pgtype.UUID `json:"pull_request_id"`
}

func (q *Queries) GetIssueVCSPullRequestLink(ctx context.Context, arg GetIssueVCSPullRequestLinkParams) (pgtype.UUID, error) {
	row := q.db.QueryRow(ctx, getIssueVCSPullRequestLink, arg.IssueID, arg.PullRequestID)
...
type GetVCSPullRequestByIDParams struct {
	ID          pgtype.UUID `json:"id"`
	WorkspaceID pgtype.UUID `json:"workspace_id"`
}

func (q *Queries) GetVCSPullRequestByID(ctx context.Context, arg GetVCSPullRequestByIDParams) (VcsPullRequest, error) {
	row := q.db.QueryRow(ctx, getVCSPullRequestByID, arg.ID, arg.WorkspaceID)
